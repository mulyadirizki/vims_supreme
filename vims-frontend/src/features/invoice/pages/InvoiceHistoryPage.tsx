...
export default function InvoiceHistoryPage() {
  ...
  const handleSearch = () => {
    setFilter((prev) => ({ ...prev, invoice_receipt_no: searchInput || undefined, page: 1 }));
  };
          ...
          <Table className="min-w-[1100px] w-full">
            ...
            <TableBody>
              ...
              {!isLoading &&
                !isError &&
                list.map((item) => (
                  <TableRow key={item.invoiceReceiptNo} className="hover:bg-blue-50/30 transition-colors border-b border-gray-50">
                    <TableCell className="text-[11px] font-semibold text-blue-600 whitespace-nowrap py-2.5">
                      {item.invoiceReceiptNo}
                    </TableCell>
                    <TableCell className="text-[11px] text-gray-600 whitespace-nowrap py-2.5">{item.grNo}</TableCell>
                    <TableCell className="text-[11px] text-gray-600 whitespace-nowrap py-2.5">{item.poNo}</TableCell>
                    ...
                    <TableCell className="text-[11px] whitespace-nowrap py-2.5">
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-gray-100 bg-white shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1">
              <Button variant="outline" size="icon" className="h-7 w-7 border-gray-200" onClick={() => goToPage(Math.max(1, page - 1))} disabled={page <= 1 || isLoading}>
                <ChevronLeft className="w-3.5 h-3.5" />
              </Button>
              <span className="text-[11px] text-gray-500 min-w-[60px] text-center">{page} / {totalPages}</span>
              <Button variant="outline" size="icon" className="h-7 w-7 border-gray-200" onClick={() => goToPage(Math.min(totalPages, page + 1))} disabled={page >= totalPages || isLoading}>
